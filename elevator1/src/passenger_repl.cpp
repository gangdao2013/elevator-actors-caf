...
namespace passenger
{
	...
	{
		auto self = scoped_actor{ system_ };
		self->request(passenger_, infinite, elevator::get_current_floor_atom::value).receive
	...
	std::string passenger_repl::get_current_state_name()
	{
		...
		self->request(passenger_, infinite, elevator::get_current_state_name_atom::value).receive
			//self->request(p, infinite, elevator::get_current_passenger_floor_atom::value).receive
			(
